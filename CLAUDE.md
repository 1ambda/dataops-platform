...
3. **Follow Architecture Patterns** - Use the hexagonal architecture patterns defined above for basecamp-server
4. **Refer to Makefile for commands** - Development workflows are automated in the Makefile
5. **Port awareness** - basecamp-server uses different ports in local (8080) vs Docker (8081) modes
6. **Multi-module structure** - basecamp-server is a multi-module Gradle project; understand the module boundaries
7. **Python package manager** - Use `uv` (not pip/poetry) for Python projects
8. **Repository Implementation** - Always follow the Repository patterns: domain interfaces → infrastructure implementations → Spring Data composition
9. **Service Implementation** - Services are concrete classes, not interfaces
10. **Keycloak port conflict** - Keycloak uses 8080, hence basecamp-server uses 8081 in full Docker mode
