...
import com.github.lambda.domain.model.query.QueryScopeFilter
import com.github.lambda.domain.repository.query.QueryExecutionRepositoryDsl
import com.github.lambda.domain.repository.query.QueryExecutionRepositoryJpa
import io.mockk.*
 ...
 *
 * Tests business logic for query management including access control,
 * query filtering, and cancellation logic. Uses MockK to isolate dependencies.
 */
@DisplayName("QueryService Unit Tests")
class QueryServiceTest {
    ...
    inner class CancelQuery {
        ...
        fun `should cancel a running query successfully`() {
            ...
            // When
            val result = queryService.cancelQuery(command, currentUser)
        ...
        fun `should throw QueryNotFoundException when query does not exist`() {
            ...
            val exception =
                assertThrows<QueryNotFoundException> {
                    queryService.cancelQuery(command, currentUser)
                }
        ...
        fun `should throw AccessDeniedException when user tries to cancel other user's query`() {
            ...
            assertThat(exception.message).contains("Access denied to other users' query")
            verify(exactly = 1) { queryExecutionRepositoryJpa.findById("query_test_001") }
            verify(exactly = 0) { queryExecutionRepositoryJpa.save(any()) }
        }
        ...
        fun `should throw AccessDeniedException when user tries to cancel system query`() {
            ...
            assertThat(exception.message).contains("Access denied to system query")
    ...
    inner class ListQueries {
        ...
        fun `should list queries with MY scope successfully`() {
            ...
            // When
            val result = queryService.listQueries(query, currentUser)
    ...
    inner class GetQueryDetails {
        ...
        fun `should return query details for owner`() {
            ...
            // When
            val result = queryService.getQueryDetails(queryId, currentUser)
        ...
        fun `should throw AccessDeniedException when user tries to view other user's query`() {
            ...
            val exception =
                assertThrows<AccessDeniedException> {
                    queryService.getQueryDetails(queryId, currentUser)
                }

            assertThat(exception.message).contains("Access denied to query execution history for action: view")
            verify(exactly = 1) { queryExecutionRepositoryJpa.findById(queryId) }
        }
    }
    ...
    inner class GetQueryDetailsOrThrow {
        ...
        fun `should return query when found`() {
            ...
            // When
            val result = queryService.getQueryDetailsOrThrow(queryId, currentUser)
        ...
        fun `should throw QueryNotFoundException when query not found`() {
            ...
            val exception =
                assertThrows<QueryNotFoundException> {
                    queryService.getQueryDetailsOrThrow(queryId, currentUser)
                }
    ...
    inner class CountQueries {
        ...
        fun `should count queries with filters correctly`() {
            ...
            // When
            val result = queryService.countQueries(query, currentUser)
    ...
    inner class GetRunningQueries {
        ...
        fun `should return running queries for specific user`() {
            ...
            // When
            val result = queryService.getRunningQueries(currentUser)
